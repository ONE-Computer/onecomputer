...
use crate::policy::{PolicyAction, PolicyRule, PolicyScope};
...
impl PolicyEngine {
    ...
    ///
    /// Implements the strictest-wins cross-scope merge:
    /// - org-scope rules (scope=organization) set the minimum floor for all agents
    ///   in the organization; they cannot be weakened by project or agent rules.
    /// - project-scope rules apply to all agents in the project.
    /// - agent-scope rules (project rows with a specific agent_id) apply to one agent.
    ///
    /// The merged slice is passed to [`crate::policy::evaluate`] which applies
    /// Block > ManualApproval > RateLimit > Allow priority (strictest action wins).
    async fn resolve_policy_rules(
        ...
        // Tag org rows with Organization scope; project rows are Project or Agent
        // depending on whether agent_id is set on the DB row.
        let org_rows = org_result
            .map_err(db_err)?
            .into_iter()
            .map(|r| (r, PolicyScope::Organization));
        let project_rows = project_result.map_err(db_err)?.into_iter().map(|r| {
            let scope = if r.agent_id.is_some() {
                PolicyScope::Agent
            } else {
                PolicyScope::Project
            };
        ...
        let rules = org_rows
            .chain(project_rows)
            .filter(|(r, _scope)| {
                // Keep rules for this host and this agent (or all agents).
