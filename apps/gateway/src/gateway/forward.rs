...
/// Stamp `x-onecli-channel-id` / `x-onecli-channel-name` response headers when
/// the inbound path matched a registered channel, so downstream clients can
/// attribute the response to the named connector. No-op when no channel matched.
fn stamp_channel_headers<B>(
    response: &mut hyper::Response<B>,
    channel: Option<&crate::channel::Channel>,
) {
    use hyper::header::HeaderValue;
    if let Some(ch) = channel {
        if let Ok(v) = HeaderValue::from_str(&ch.id) {
            response.headers_mut().insert("x-onecli-channel-id", v);
        }
        if let Ok(v) = HeaderValue::from_str(&ch.name) {
            response.headers_mut().insert("x-onecli-channel-name", v);
        }
    }
}
...
pub(crate) async fn forward_request(
    ...
    vti_signing_key: &Arc<affinidi_secrets_resolver::secrets::Secret>,
    ...
    // LLM upstream rewrite: when the operator configures a local LLM gateway
    // (LiteLLM) via `LLM_UPSTREAM_URL` + `LLM_UPSTREAM_KEY`, requests to the
    // trigger host (default `api.anthropic.com`) are rewritten to that upstream
    // with the upstream key injected as `x-api-key`. This is what lets Claude
    // Code's stock `ANTHROPIC_BASE_URL=https://api.anthropic.com` config flow
    // through the gateway's MITM + policy path while the actual call lands on
    // LiteLLM — instead of 401 `credential_not_found` because no Anthropic
    // credential is configured in the vault. Inert when the env vars are unset.
    let llm_rewrite =
        super::llm_upstream::LlmUpstream::from_env().and_then(|u| u.rewrite(host).map(|r| (u, r)));
    let (effective_scheme, effective_host) = match &llm_rewrite {
        Some((_, r)) => {
            super::llm_upstream::log_rewrite(host, &r.scheme, &r.authority);
            (r.scheme.as_str(), r.authority.as_str())
        }
        None => (scheme, host),
    };

    let url = format!("{effective_scheme}://{effective_host}{path}");
    let agent_token = proxy_ctx.agent_token.as_deref().unwrap_or("");

    // Channel routing: match the inbound path prefix to a named connector.
    // When a channel matches, its id/name are stamped onto the response and an
    // MCP protocol enables request-body buffering so G2's MCP parser can
    // inspect JSON-RPC `tools/call` requests for per-tool policy conditions.
    let channel = channel_registry.match_path(&path);
    let channel_mcp = channel.is_some_and(|ch| ch.protocol == crate::channel::ChannelProtocol::Mcp);
    ...
    // overhead unless a default interception matched. An MCP-matched channel
    // also forces buffering so G2's MCP JSON-RPC parser can inspect the body.
    let (mut condition_buffer, mut req) = if llm_rewrite.is_some() {
        // LLM upstream active: fully buffer the body so DLP can scan/redact it
        // before forwarding to LiteLLM.
        let (parts, incoming) = req.into_parts();
        let bytes = incoming
            .collect()
            .await
            .context("buffering LLM request body for DLP")?
            .to_bytes();
        let req = hyper::Request::from_parts(parts, reqwest::Body::from(bytes.clone()));
        (Some(bytes.to_vec()), req)
    } else if crate::condition_match::needs_body_buffer(&rules.policy_rules)
        || hooks::needs_request_body(rules, host, method.as_str(), &path)
        || channel_mcp
    {
        let (parts, incoming) = req.into_parts();
        let (buf, fwd_body) = crate::condition_match::prepare_body(
            incoming,
            method.as_str(),
            &url,
            &rules.policy_rules,
    ...
    // ── DLP scan: when the LLM upstream rewrite is active (i.e. this request
    // is bound for LiteLLM), scan the buffered body for PII / sensitive data
    // via Microsoft Presidio. If DLP_REDACT=true, redact PII before forwarding.
    // Best-effort: never blocks the request on DLP failure. Inert when
    // PRESIDIO_ANALYZER_URL is unset.
    if llm_rewrite.is_some() {
        if let Some(body_bytes) = condition_buffer.as_ref() {
            if let Ok(text) = std::str::from_utf8(body_bytes) {
                let (scanned_body, findings) = super::dlp::scan_and_redact(text).await;
                if !findings.is_empty() {
                    let types: Vec<&str> =
                        findings.iter().map(|f| f.entity_type.as_str()).collect();
                    let redacted = scanned_body != text;
                    tracing::info!(
                        host = %host,
                        path = %path,
                        count = findings.len(),
                        types = ?types,
                        redacted,
                        "DLP: PII detected in outbound LLM request"
                    );
                    let alert = super::dlp::DlpAlertBody {
                        organization_id: proxy_ctx
                            .organization_id
                            .clone()
                            .unwrap_or_else(|| "unknown".to_string()),
                        project_id: proxy_ctx.project_id.clone(),
                        sandbox_id: None,
                        agent_id: proxy_ctx.agent_id.clone(),
                        approval_id: None,
                        request_log_id: None,
                        source: "gateway".to_string(),
                        direction: "outbound_prompt".to_string(),
                        host: Some(host.to_string()),
                        path: Some(path.clone()),
                        method: Some(method.to_string()),
                        action: if redacted { "redact" } else { "observe" }.to_string(),
                        risk_level: super::dlp::risk_level(&findings),
                        entity_types: super::dlp::findings_json(&findings),
                        finding_count: findings.len(),
                        redacted,
                        blocked: false,
                        sample_hash: Some(super::dlp::sha256_hex(text)),
                        metadata: Some(serde_json::json!({
                            "destinationType": "approved_llm",
                            "provider": "litellm",
                            "bodyBytes": text.len(),
                        })),
                    };
                    super::dlp::notify_api(&http_client, &alert).await;
                    // If redacted, replace BOTH the buffer AND the forwarded
                    // request body so the model receives the redacted prompt.
                    if scanned_body != text {
                        if let Some(cb) = condition_buffer.as_mut() {
                            cb.clear();
                            cb.extend_from_slice(scanned_body.as_bytes());
                        }
                        let redacted_bytes = scanned_body.into_bytes();
                        req = hyper::Request::from_parts(
                            req.into_parts().0,
                            reqwest::Body::from(redacted_bytes),
                        );
                    }
                }
            }
    ...
    match &decision {
        ...
        PolicyDecision::Blocked { rule_name } => {
            ...
            metrics::record_blocked(rule_name);
    ...
    // When the LLM upstream rewrite is active, its rule (set `x-api-key`,
    // remove `authorization`) is appended so the upstream key is injected
    // alongside any vault/app-connection rules already resolved.
    let mut upstream_path = path.clone();
    let llm_rewrite_rule = llm_rewrite
        .as_ref()
        .map(|(_, r)| super::llm_upstream::rewrite_rule(r));
    let injection_count = match &llm_rewrite_rule {
        Some(rule) => {
            let mut combined = rules.injection_rules.clone();
            combined.push(rule.clone());
            inject::apply_injections(&mut headers, &mut upstream_path, &combined)
        }
        None => inject::apply_injections(&mut headers, &mut upstream_path, &rules.injection_rules),
    };
    let upstream_url = format!("{effective_scheme}://{effective_host}{upstream_path}");
    ...
    let (forward_body, approval_log_id, approval_id_for_telemetry, approval_triggered_at) =
        if let PolicyDecision::ManualApproval { rule_id } = &decision {
            ...
            // Persist a durable ApprovalRequest in the ONEComputer API so the
            // manager can see it in the queue.  Awaited here (before the
            // decision wait) so the record exists before the SDK polls.
            // Fail-closed-with-log: any error is warned and the in-memory hold
            // continues normally per design 14-A.
            super::approval_notify::notify_api(&http_client, &approval, rule_id).await;

            // Race the in-process watch channel (woken by the SDK long-poll
            // path, used in tests) against the HTTP poll loop (production
            // wake source: manager decides via the API).  Whichever fires
            // first wins; the 180s hard timeout is the backstop.
            let approval_decision = tokio::select! {
                // In-process path: SDK or in-proc test submitted a decision.
                d = decision_rx.wait(Duration::from_secs(APPROVAL_TIMEOUT_SECS)) => d,
                // Production path: poll the API until approved/denied/expired.
                // The signing key is passed so the poll loop can verify the
                // signed decision VC (context._vti.decision) against the
                // gateway's did:web public key BEFORE releasing on "approved"
                // — the signature is load-bearing, not decorative (ONE-142).
                d = super::approval_poll::poll_for_decision(
                    &http_client,
                    &approval_id,
                    org_id,
                    project_id,
                    approval_store,
                    Some(vti_signing_key.as_ref()),
    ...
    if injection_count == 0 && status == StatusCode::BAD_REQUEST && proxy_ctx.agent_token.is_some()
    {
        ...
        *response.status_mut() = status;
        ...
        stamp_channel_headers(&mut response, channel);
        return Ok(response);
    }
    ...
    // ── VP injection (Phase 3 G3) ─────────────────────────────────────────
    // When ONECLI_VP_INJECTION=true and the upstream response is JSON on an
    // MCP-matched channel, buffer the body, build a signed
    // AgentIdentityCredential VP, and inject it at result._meta.agentIdentity.
    // Falls back to streaming on any miss (non-JSON, non-MCP, non-JSON-RPC,
    // signing failure). Default off in dev — set the env var to opt in.
    if channel_mcp
        && std::env::var("ONECLI_VP_INJECTION").as_deref() == Ok("true")
        && content_type.starts_with("application/json")
    {
        match upstream_resp.bytes().await {
            Ok(body_bytes) => {
                let mut body_value: serde_json::Value = match serde_json::from_slice(&body_bytes) {
                    Ok(v) => v,
                    Err(e) => {
                        warn!(url = %url, error = %e, "VP injection: body not valid JSON — streaming as-is");
                        let mut response = Response::new(Either::Left(Full::new(body_bytes)));
                        *response.status_mut() = status;
                        for (name, value) in resp_headers.iter() {
                            if is_forwarded_response_header(name) {
                ...
                // Build the agent-identity VP and inject it. A signing error
                // or a non-JSON-RPC body falls back to the original bytes so the
                // response is never corrupted by a VP-injection failure.
                let issuer_did = vti_signing_key
                    .id
                    .split('#')
                    .next()
                    .unwrap_or(&vti_signing_key.id)
                    .to_string();
                let channel_name = channel.map(|ch| ch.name.clone()).unwrap_or_default();
                let issued_at = time::OffsetDateTime::now_utc()
                    .format(&time::format_description::well_known::Rfc3339)
                    .unwrap_or_default();
                let credential = crate::identity_injection::AgentIdentityCredential {
                    issuer_did: issuer_did.clone(),
                    channel_name,
                    issued_at,
                };

                match crate::identity_injection::build_agent_vp(
                    &credential,
                    vti_signing_key.as_ref(),
                    &issuer_did,
                )
                .await
                {
                    Ok(vp) => {
                        crate::identity_injection::inject_vp_into_response(&mut body_value, vp);
                        let injected_bytes =
                            serde_json::to_vec(&body_value).unwrap_or_else(|_| body_bytes.to_vec());
                        let injected_len = injected_bytes.len();
                        let mut response =
                            Response::new(Either::Left(Full::new(Bytes::from(injected_bytes))));
                        *response.status_mut() = status;
                        for (name, value) in resp_headers.iter() {
                            if is_forwarded_response_header(name) {
                                response.headers_mut().append(name.clone(), value.clone());
                        ...
                        // Recompute content-length for the rewritten body so the
                        // client receives a correct framing header after injection.
                        if let Ok(cl) =
                            hyper::header::HeaderValue::from_str(&injected_len.to_string())
                        {
                            response
                                .headers_mut()
                                .insert(hyper::header::CONTENT_LENGTH, cl);
                        }
                        stamp_channel_headers(&mut response, channel);
                        info!(url = %url, "VP injected into MCP JSON-RPC response");
                        // Record metrics for the (now-buffered) proxied request.
                        let hostname = super::strip_port(host);
                        let (provider, _) =
                            crate::apps::provider_for_host_and_path(hostname, &path)
                                .unwrap_or((hostname, hostname));
                        metrics::record_request(
                            method.as_str(),
                            status.as_u16(),
                        ...
                        if injection_count > 0 {
                            metrics::record_injection(provider);
                        }
                        return Ok(response);
                    }
                    Err(e) => {
                        warn!(url = %url, error = %e, "VP signing failed — streaming original body");
                        let mut response = Response::new(Either::Left(Full::new(body_bytes)));
                        *response.status_mut() = status;
                        for (name, value) in resp_headers.iter() {
                            if is_forwarded_response_header(name) {
            ...
            Err(e) => {
                warn!(url = %url, error = %e, "buffering upstream body for VP injection failed — falling back to stream");
                // `upstream_resp` was consumed by the failed bytes() call so we
                // cannot stream it. Return a 502 to signal the failure honestly.
                return Ok(response::json(
                    StatusCode::BAD_GATEWAY,
                    serde_json::json!({
                        "error": "upstream body read failed during VP injection",
                    }),
                ));
            }
        }
    ...
    *response.status_mut() = status;
    ...
    // Record the proxied request with its upstream status + round-trip latency.
    // If any credential injections were applied, count them by provider.
    let hostname = super::strip_port(host);
    let (provider, _) =
