 ...
 *
 * Prefers a project the user created (their "own" default), then falls back to
 * the first project in their first organization — so members who didn't create
 * the team's project (e.g. seeded Demo Corp roles under ONE-125) still resolve
 * to a usable default project instead of falling through to org bootstrap.
 */
export const findUserDefaultProject = async (
  ...
  const ownProject = await db.project.findFirst({
    where: {
