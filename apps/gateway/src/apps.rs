...
static APP_PROVIDERS: &[AppProvider] = &[
    AppProvider {
        ...
        display_name: "Microsoft Graph (Outlook)",
        host_rules: &[HostRule {
...
mod tests {
    ...
    #[test]
    fn microsoft_graph_is_a_bearer_injection_provider() {
        assert_eq!(
            providers_for_host("graph.microsoft.com"),
            vec!["microsoft-graph"]
        );
        assert!(needs_access_token("microsoft-graph"));
        assert_eq!(
            build_app_injections(
                "microsoft-graph",
                "graph.microsoft.com",
                "graph-token"
            ),
            vec![Injection::SetHeader {
                name: "authorization".to_string(),
                value: "Bearer graph-token".to_string(),
            }]
        );
    }
