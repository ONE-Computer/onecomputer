...
// MCP JSON-RPC 2.0 body parsing for per-tool policy conditions (mcp_tool:*).
mod mcp;
...
// VTI (Verifiable Task Identifier) signer: did:web identity + W3C VC signing
// via the affinidi TDK (eddsa-jcs-2022 Data Integrity proofs). No custom crypto.
mod vti_signer;

// Agent-identity VP injection (Phase 3 G3): builds a signed
// AgentIdentityCredential VP and injects it into MCP JSON-RPC responses under
// result._meta.agentIdentity. Signing is delegated to vti_signer (affinidi SDK).
mod identity_injection;
...
async fn main() -> Result<()> {
    ...
    // Load the VTI signing key (Ed25519) for did:web identity + VC signing.
    // From ONECLI_GATEWAY_SIGNING_KEY (base64 seed) when set, else ephemeral.
    // Held in an Arc for the per-request VP-injection layer (Phase 3) to share.
    let vti_signing_key: Arc<affinidi_secrets_resolver::secrets::Secret> =
        Arc::new(vti_signer::load_signing_key()?);
    info!(
        did = %vti_signer::gateway_did(
            &std::env::var("ONECLI_GATEWAY_PUBLIC_URL")
                .unwrap_or_else(|_| "localhost".to_string())
        ),
        "VTI signing key loaded"
    );
    ...
    // Load the channel registry from ONECLI_CHANNELS (JSON array). Empty when
    // unset. Shared across all requests so policy rules and per-channel auth
    // (Phase 3 VP injection) can reference channel names.
    let channel_registry = Arc::new(channel::ChannelRegistry::from_env());
    info!(
        channels = channel_registry.channels.len(),
