...
export const kasmLocalProvider: SandboxProvider = {
  ...
  async bootstrapSandbox(
    ...
    const install = await installClaudeCode(id);
    if (install.exitCode !== 0) {
      throw new Error("Claude Code bootstrap failed");
    }
    // Desktop is optional for headless agent workloads. Its package repository
    // must never prevent the required Claude Code runtime from becoming ready.
    const installDesktop =
      process.env.ONECOMPUTER_INSTALL_CLAUDE_DESKTOP === "true";
    if (installDesktop) await installClaudeDesktop(id);
    ...
    const desktop3p = installDesktop
      ? await configureClaudeDesktop3p(id)
      : { exitCode: 1, output: "Claude Desktop disabled for headless runtime" };
...
LLM proxy: ${proxy.output}`.slice(-4000),
  ...
  ensureVisualRuntime: (id) =>
    ensureVisualRuntime(id, execInSandbox, {
      display: ":1",
      width: 1440,
      height: 900,
    }),
  captureScreenshot: (id) =>
    captureX11Screenshot(id, execInSandbox, {
      display: ":1",
      width: 1440,
      height: 900,
    }),
