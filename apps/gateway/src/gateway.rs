...
pub(crate) struct GatewayState {
    ...
    /// Inbound path-prefix → connector routing table (loaded from ONECLI_CHANNELS).
    pub channel_registry: Arc<crate::channel::ChannelRegistry>,
    /// Gateway Ed25519 signing key for VP injection (Phase 3 G3). Held in an
    /// `Arc` so every request can share it when `ONECLI_VP_INJECTION=true`.
    pub vti_signing_key: Arc<affinidi_secrets_resolver::secrets::Secret>,
}
...
impl GatewayServer {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        let axum_router = Router::new()
            ...
            // did:web DID document — resolves the gateway's signing key over
            // HTTPS so third parties (and the API's verify-on-read) can verify
            // gateway-issued VCs without trusting an in-process build. VTI STEP 2.
            .route(
                "/.well-known/did.json",
                axum::routing::get(well_known_did_json),
            )
            .route(
                "/metrics",
                axum::routing::get(|| async { crate::metrics::render_metrics() }),
            )
...
/// Serve the gateway's did:web DID document at `/.well-known/did.json`.
///
/// Resolves the gateway's configured public URL to a `did:web:<host>` and
/// exposes the Ed25519 signing key's public half as a Multikey verification
/// method. The document is built with [`crate::vti_signer::build_did_doc`]
/// using the same `vti_signing_key` the gateway signs VCs with, so the key
/// advertised here is exactly the key that Data Integrity proofs reference.
///
/// Public + unauthenticated per the did:web spec: DID documents are
/// self-certifying and intended to be resolvable by anyone.
async fn well_known_did_json(State(state): State<GatewayState>) -> axum::response::Response {
    let base_url =
        std::env::var("ONECLI_GATEWAY_PUBLIC_URL").unwrap_or_else(|_| "localhost".to_string());
    let did = crate::vti_signer::gateway_did(&base_url);
    match crate::vti_signer::build_did_doc(&state.vti_signing_key, &did) {
        Ok(doc) => (
            [(hyper::header::CONTENT_TYPE, "application/did+json")],
            axum::Json(doc),
        )
            .into_response(),
        Err(e) => {
            warn!(error = ?e, did = %did, "failed to build DID document");
                ...
                Ok(upgraded) => {
                    let result = if intercept {
                        mitm::mitm(
                            ...
                            Arc::clone(&state.vti_signing_key),
                        )
                        .await
                    } else {
