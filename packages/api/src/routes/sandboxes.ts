...
export const sandboxRoutes = () => {
  ...
  // POST /sandboxes/:id/visual/start — prepare a headless X11 runtime.
  // This is intentionally independent of VNC: Xvfb owns the display,
  // Chromium is controlled through loopback-only CDP, and screenshots leave
  // the sandbox only through the authenticated APIs below.
  app.post("/:id/visual/start", async (c) => {
    const auth = c.get("auth");
    const ability = c.get("ability");
    const id = c.req.param("id");
    const ownerId = await assertSandboxOwner(auth, id);
    ...
    const status = await withAudit(
      () => getSandboxProvider().ensureVisualRuntime(id),
      () => ({
        organizationId: auth.organizationId,
        userId: auth.userId,
        userEmail: auth.userEmail,
        ...
        service: AUDIT_SERVICES.SANDBOX,
        source: AUDIT_SOURCE.API,
        metadata: { sandboxId: id, target: "x11-visual-runtime" },
      }),
    );
    return c.json(status);
  });

  // GET /sandboxes/:id/visual/screenshot — one authenticated PNG frame.
  app.get("/:id/visual/screenshot", async (c) => {
    const auth = c.get("auth");
    const ability = c.get("ability");
    const id = c.req.param("id");
    const ownerId = await assertSandboxOwner(auth, id);
    requireAbility(ability, "read", subject("Sandbox", { id, ownerId }));
    const frame = await getSandboxProvider().captureScreenshot(id);
    ...
    return new Response(png, {
      headers: {
        ...
        "X-OneComputer-Captured-At": frame.capturedAt ?? "",
        "X-Content-Type-Options": "nosniff",
      },
    });
  });

  // GET /sandboxes/:id/visual/stream — low-latency multipart PNG stream.
  // The first implementation targets ~1.3 FPS. WebRTC can replace this
  // transport later without changing the capture or authorization boundary.
  app.get("/:id/visual/stream", async (c) => {
    const auth = c.get("auth");
    const ability = c.get("ability");
    const id = c.req.param("id");
    const ownerId = await assertSandboxOwner(auth, id);
    ...
    const boundary = "onecomputer-frame";
  ...
  app.get("/:id", async (c) => {
    ...
    const sandbox = await mergeSandboxInfo(row);
