...
/**
 * True when at least one OAuth provider is fully configured.
 *
 * OAuth mode is activated by setting NEXTAUTH_SECRET (see runtime-config.ts).
 * That mode is only usable if a provider can actually sign users in — either
 * Google (GOOGLE_CLIENT_ID) or Microsoft Entra ID (all three AZURE_AD_* vars).
 * Without any provider, the login page has no working sign-in button, so we
 * surface that as `oauth-misconfigured` rather than rendering a dead end.
 */
...
const getSetupError = (): SetupErrorCode | null => {
  ...
  // NEXTAUTH_SECRET is set but no OAuth provider is fully configured.
  if (NEXTAUTH_SECRET && !hasOAuthProvider()) {
