...
import type { RuleLevel } from "@/app/(dashboard)/rules/_components/types";
import {
  listPolicyRules,
...
export const createRule = async (
  ...
  // "enterprise" level → org scope so the rule acts as the floor for all teams.
  const scope = level === "enterprise" ? { organizationId } : { projectId };
  return withAudit(
    () => createPolicyRuleService(scope, input),
    (rule) => ({
      ...(level === "enterprise" ? { organizationId } : { projectId }),
      userId,
      userEmail,
