...
Implementation is present on `mike/openvtc-push-notifications` as of
2026-07-23. The owned `/companion` PWA, generic Web Push adapter, encrypted
...
The issue remains blocked and is not complete. Issue 003 has not yet landed,
runtime VAPID secrets have not been provisioned in this worktree, and the
required physical browser/platform, background delivery, authenticator,
provider outage, restart, storage inspection, and screenshot evidence has not
been collected. See `infra/issue-004/README.md` for the implemented protocol,
support boundary, pins, minimized payload, and remaining verification.
