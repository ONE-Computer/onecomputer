...
export const appRoutes = () => {
  ...
  app.get("/:provider/callback", async (c) => {
    ...
    try {
      ...
      // NOTE: not wrapped in withAudit — this is an unauthenticated OAuth
      // provider callback (no authMiddleware, no session). Neither the
      // request nor the signed OAuthStatePayload carries a userId/userEmail,
      // so there is no real actor identity to attribute the audit record to.
      // The POST /:provider/connect path below (same underlying service
      // calls, but under an authenticated session) is audited.
    ...
    if (body.connectionId) {
      await withAudit(
        ...
        () => ({
          ...
          service: AUDIT_SERVICES.APP_CONNECTION,
          source: AUDIT_SOURCE.API,
          metadata: { connectionId: body.connectionId, provider },
        }),
      );
    } else {
      ...
      if (duplicate) {
        await withAudit(
          ...
          () => ({
            ...
            service: AUDIT_SERVICES.APP_CONNECTION,
            source: AUDIT_SOURCE.API,
            metadata: { connectionId: duplicate.id, provider },
          }),
        );
      } else {
        ...
        await withAudit(
          ...
          (result) => ({
            ...
            service: AUDIT_SERVICES.APP_CONNECTION,
            source: AUDIT_SOURCE.API,
            metadata: { connectionId: result.id, provider },
          }),
        );
      }
    }
