...
/**
 * Provisions a did:web identity for an agent.
 *
 * Uses Node's built-in `generateKeyPairSync` (NOT custom Ed25519 math) to
 * derive an Ed25519 keypair. The private key is intentionally not persisted
 * here — see TODO below. The public key is exported as SPKI/DER hex so it can
 * be published in a did:web DID document later.
 */
function provisionAgentDid(agentId: string): {
  did: string;
  publicKeyHex: string;
} {
...
export const createAgent = async (
  ...
  try {
    ...
    // Provision a did:web identity now that the agent row (and its id) exists.
    const { did, publicKeyHex } = provisionAgentDid(agent.id);
    await db.agent.update({
      where: { id: agent.id },
      data: { did, didPublicKey: publicKeyHex },
    });

    logger.info(
      { agentId: agent.id, did },
      "Provisioned DID for agent %s: %s",
      agent.id,
      did,
    );
...
/**
 * Kill switch — revoke an agent's access token so it can no longer
 * authenticate to the gateway. The token is replaced with a `revoked_`
 * sentinel (rather than null) so the unique constraint is preserved and the
 * revocation is forensically visible in the DB row. The previous live token
 * is captured in metadata for audit.
 */
export const revokeAgentToken = async (
  ...
  const previousTokenPrefix = agent.accessToken
    ? agent.accessToken.slice(0, 4) // "aoc_"
    : null;

  await db.agent.update({
    where: { id: agentId },
    data: { accessToken: `revoked_${Date.now()}` },
  });

  return { agentId, previousTokenPrefix, reason };
};
